...
function Individual({articulo}){
    ...
    return(
        <div className="articulo">
            <img className="articulo-img" src={articulo?.images[0]}
                 alt={articulo?.name}></img>
            <div className="articulo-info">
                <p className="text-title">{articulo?.name}</p>
                <p className="text-body">{articulo?.description}</p>
            </div>
            <div className="articulo-footer">
                <span className="text-title">$ {articulo?.price.unit_amount / 100} {articulo?.price.currency}</span>
                <button onClick={addToCart} className="articulo-button">
